...
namespace GetSheetset
{
    ...
    public class MyCommands
    {
        ...
        public void ListLayouts()
        {
            ...
            if (inifile.ExistINIFile())
            {
                ...
                try
                {
                    ...
                    Patten_Date = inifile.IniReadValue("RegExp", "date");
                     ...
                     Patten_Fifth = inifile.IniReadValue("RegExp", "fifth");
                }
            ...
            Regex Reg_Fifth = new Regex(Patten_Fifth);
            foreach (string text in TextList)
            {
                ...
                if (Reg_Fifth.IsMatch(text))
                {
                    SheetSet["fifth"] = Reg_Fifth.Match(text).ToString().Trim();
                }

            }
            SheetSet["sheetsetname"] = string.Format("{0} {1} {2} {3} {4} {5}", SheetSet["period"], SheetSet["first"], SheetSet["second"], SheetSet["third"], SheetSet["fourth"], SheetSet["fifth"]);
            //测试用代码
            #region testcode
            acDoc.Editor.WriteMessage("\n该图册信息:");
